...
of the public keys (encapsulation keys) from the Subject Public Key format
used in X.509 certificates and in bare public keys.
"""
...
def ek_from_der(enc_key):
    ...
    if empty != b"":
        raise der.UnexpectedDER("Trailing just after DER public key")
...
def dk_from_der(enc_key):
    """
    Extract encapsulation and decapsulation keys from PKCS #8 encoding.
